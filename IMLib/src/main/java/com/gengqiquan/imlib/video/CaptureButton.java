...
import com.gengqiquan.imlib.video.listener.CaptureListener;
import com.gengqiquan.imlib.video.util.CheckPermission;
...
public class CaptureButton extends View {
    ...
    public CaptureButton(Context context, int size) {
        ...
        button_state = ImCameraView.BUTTON_STATE_BOTH;  //初始化按钮为可录制可拍照
    ...
    public boolean onTouchEvent(MotionEvent event) {
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                ...
                //判断按钮状态是否为可录制状态
                if ((button_state == ImCameraView.BUTTON_STATE_ONLY_RECORDER || button_state == ImCameraView.BUTTON_STATE_BOTH))
                    postDelayed(longPressRunnable, 500);    //同时延长500启动长按后处理的逻辑Runnable
                break;
            case MotionEvent.ACTION_MOVE:
                if (captureLisenter != null
                        && state == STATE_RECORDERING
                        && (button_state == ImCameraView.BUTTON_STATE_ONLY_RECORDER || button_state == ImCameraView.BUTTON_STATE_BOTH)) {
    ...
    private void handlerUnpressByState() {
        ...
        switch (state) {
            //当前是点击按下
            case STATE_PRESS:
                if (captureLisenter != null && (button_state == ImCameraView.BUTTON_STATE_ONLY_CAPTURE || button_state ==
                        ImCameraView.BUTTON_STATE_BOTH)) {
